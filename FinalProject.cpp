...
const double MIN_SPEED_SQ = 5.0 * 5.0; 


int CURRENT_NUM_BALLS = 0; 
...
public:
    ...
    void constrain_bounds() {
        ...
        Vec2 current_vel = get_velocity();
        if (current_vel.lengthSq() < MIN_SPEED_SQ) {
            Vec2 boost_dir = current_vel.normalize();
            if (boost_dir.lengthSq() == 0) {
                std::random_device rd;
                std::mt19937 gen(rd());
                std::uniform_real_distribution<> dis_angle(0.0, 2.0 * M_PI);
                double angle = dis_angle(gen);
                boost_dir = Vec2(std::cos(angle), std::sin(angle));
            }
            set_velocity(boost_dir * std::sqrt(MIN_SPEED_SQ));
        }
    }
};
...
void initialize_balls(std::vector<Ball*>& balls, int num_balls_to_create, std::mt19937& gen, std::uniform_real_distribution<>& dis_pos) {
    ...
    for (int i = 0; i < num_balls_to_create; ++i) {
...
int main() {
    ...
    std::cout << "--------------------------------------------------------" << std::endl;
    std::cout << "SIMULASI KINERJA ALGORITMA: SWITCHING REAL-TIME" << std::endl;
    std::cout << "Masukkan JUMLAH BOLA yang ingin disimulasikan (Misal: 30, 100, 500): ";
    
    if (!(std::cin >> new_num_balls) || new_num_balls <= 0) {
        std::cerr << "Input tidak valid. Menggunakan default 67 bola." << std::endl;
        new_num_balls = 67;
    }
    
    CURRENT_NUM_BALLS = new_num_balls;
    
    std::cout << "--------------------------------------------------------" << std::endl;
    std::cout << "KONTROL: Tekan 'Q' untuk Quadtree, Tekan 'B' untuk Brute Force." << std::endl;
    std::cout << "Memulai simulasi dengan " << CURRENT_NUM_BALLS << " bola..." << std::endl;
    std::cout << "--------------------------------------------------------" << std::endl;

    sf::RenderWindow window(sf::VideoMode(WINDOW_WIDTH, WINDOW_HEIGHT), "Simulasi Fisika: Tekan Q atau B");
    ...
    std::vector<Ball*> balls;
    initialize_balls(balls, CURRENT_NUM_BALLS, gen, dis_pos);
    ...
    bool current_use_quadtree = use_quadtree; 
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::KeyPressed) {
                if (event.key.code == sf::Keyboard::Q) {
                    current_use_quadtree = true;
                    std::cout << "SWITCHED: Menggunakan Quadtree (O(N log N))" << std::endl;
                } else if (event.key.code == sf::Keyboard::B) {
                    current_use_quadtree = false;
                    std::cout << "SWITCHED: Menggunakan Brute Force (O(N^2))" << std::endl;
                }
            }
        }
        ...
        const double INERTIA_FACTOR = -300.0; 
        ...
        while (accumulator >= fixed_update_time) {
            ...
            window.setTitle("Simulasi | N: " + std::to_string(CURRENT_NUM_BALLS) +
                            " | Algoritma: " + std::string(current_use_quadtree ? "Quadtree (Q)" : "Brute Force (B)") +
